...
Updates `pyproject.toml` and `src/csdb/__init__.py`, then checks Python source compilation. GitHub Releases build and publish `csvdatabase-csdb` to PyPI. Release verification requires Python 3.11 or newer. Configure the PyPI trusted publisher for `csvdatabase/csdb-python`, workflow `release.yml`, and environment `pypi` before publishing. The package currently contains scaffolding; the database implementation is not yet available.
